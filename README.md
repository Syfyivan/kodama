...
托盘菜单「大小」可选 很小 / 小 / 中 / 大（持久化到 `userData/kodama-window.json`，桌宠按窗口自适应缩放）。右键面板还能实时调「图案大小」「透明度」「点击范围」「触发方式」「贴边」「抚摸」「游走」「气泡位置」「面板位置」「气泡高度」「气泡间距」，设置存在 Electron 本地存储里，并支持复制/粘贴配置 JSON，也可以手动检查 / 安装更新。
