...
菜单栏 Kodama 的「今日任务 / Agent」面板会保留今日任务、最近事件、待交互项、Agent 完成项、可跳转会话和当前 bridge/hook 状态；气泡错过时可以从这里回看。桌宠旁只常驻紧凑的今日任务气泡，限定高度并支持滚动；Session 事件气泡只显示状态和当前动作，不再把 Session 进度伪装成你的任务进度。

## Agent 任务进度与 Session 归组

Kodama 不把一次 Session 等同于一个任务。任务是你今天要完成的真实事项，Session 只是被归入任务的执行过程：

- 数据层级是 `我的任务 → Session → 最近步骤`。任务进度由你在任务编辑器中独立设置，不会根据 Session 的完成比例自动计算；即使所有 Session 已完成，任务也只有在你把任务进度设为 100% 后才算完成。
- 桌宠旁显示小型任务气泡：任务名、任务百分比、Session/Todo 数量和一个下一步提示。气泡区最大高度固定，任务或 Session 多时在原位置上下滚动，不会铺满桌面。
- 尚未归组的 Session 仍以紧凑事件气泡出现，只显示来源、状态和当前动作，不显示进度条；可直接把 Session 气泡拖到某个任务气泡上。
- 右键桌宠打开的同一个管理器里统一放置「任务 / 消息 / Bridge / 会话」等入口。点击任务气泡的「编辑」才展开任务详情，用于修改名称和任务进度、维护 Todo、归组或移出 Session。
- 删除任务时只移除任务和它的 Todo，Session 会回到待归组区域，过程记录不会丢失；单个 Session 可以暂时「忽略」，也可以从管理器恢复显示。
- 每个 Session 都有「归组」菜单，可以移动到今天已有任务、回到待归组区域，或现场新建任务。
...
- 顶部「Agent 任务」展示你明确创建的今日任务；一个任务可以容纳多个并行或连续 Session，并能从最近步骤直接跳回具体会话。
...
托盘菜单「大小」可选 很小 / 小 / 中 / 大（持久化到 `userData/kodama-window.json`，桌宠按窗口自适应缩放）。右键面板还能实时调「图案大小」「小人与气泡透明度」「点击范围」「触发方式」「贴边」「抚摸」「游走」「气泡位置」「面板位置」「气泡高度」「气泡间距」，设置存在 Electron 本地存储里，并支持复制/粘贴配置 JSON，也可以手动检查 / 安装更新。任务气泡隐藏后，桌宠会进入更活跃的轻陪伴状态，偶尔做动作并显示独立的小对话泡。
...
- 改右键面板里的设置：**实时生效**，不用重启。
- 打包安装版：启动后会自动检查 GitHub Releases；托盘菜单、右键设置面板和管理中心都可以手动「检查更新」，下载完成后可点「安装更新」并重启到新版。
- 本地 dev 版（`pnpm start` / `electron .`）：不会走安装版自动更新器，因为它是源码进程，没有可替换的签名安装包和安装路径；更新 dev 版要拉代码后重启桌宠。
