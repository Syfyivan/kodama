...
const allowed = new Set(['show', 'hide', 'toggle', 'panel', 'reset-position', 'bridge-tasks', 'healthz', 'tokens', 'token-test'])
...
async function main() {
  ...
  try {
    ...
    if (!['show', 'panel', 'reset-position', 'bridge-tasks', 'tokens', 'token-test'].includes(action)) throw err
  }

  await launchDetached()
  await waitForServer()
  const result = action === 'panel' || action === 'reset-position' || action === 'bridge-tasks'
    ? await request(`/pet/${action}`)
    : action === 'tokens'
      ? await request('/pet/token-stats', 'GET')
      : action === 'token-test'
